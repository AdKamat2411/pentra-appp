...
struct UserService {
    static func fetchUser(withUID uid: String) async throws -> User {
        let snapshot = try await Firestore.firestore().collection(COLLECTION_USER).document(uid).getDocument()
        return try snapshot.data(as: User.self)
    }
    
    @MainActor
    static func fetchSwipeCardUsers(user: User) async throws -> [User] {
        // If user preference is specific, the potential user can have only that specific gender
        // AND
        // need to exclude current userid
        // Select users whose prefernce matches our gender
        let query = Firestore.firestore().collection(COLLECTION_USER)
            .whereFilter(
                Filter.andFilter([
                    Filter.whereField("gender",in: [user.preference]),
                    Filter.whereField("id", isNotEqualTo: user.id),
                    Filter.whereField("preference", in: [user.gender])
                ])
            )
